...
The `live_session_id` uniqueness index prevents duplicate review jobs for the same monitored contest session.

## `contest_reviews`

Stores the final validated contest review generated by the background worker.
...
`live_session_id` is unique so one monitored contest session produces one canonical review.

## `contest_review_execution_logs`

Stores stage transitions, progress percentages, messages, timing, and metadata for each review job.

## `contest_review_dead_letters`

Stores permanently failed review jobs after retry exhaustion, including failure reason, stack trace, retry count, last stage, and serialized failure context.

## `contest_review_metrics`

Stores worker-level operational metrics such as queue samples, completed jobs, retrying jobs, and dead-lettered jobs.

## `contest_roadmap_updates`

Stores roadmap and recommendation tracking records generated after a successful contest review.

`review_id` is unique so retries after a partial worker crash update the existing roadmap record instead of duplicating it.
