...
Canonical vocabulary used by the reasoning layer to label behavioral findings with stable concepts. See [Behavior Ontology](architecture/behavior-ontology.md).

## Reasoning Context

Deterministic, ontology-backed context produced from an Evidence Package for future LLM consumption.

## Reasoning Context Engine

Subsystem that extracts findings, builds causal chains, handles contradictions, compresses evidence, estimates confidence, and applies token budgets. See [Reasoning Context Engine](architecture/reasoning-context-engine.md).

## Prompt Package

Provider-independent prompt artifact containing system prompt, developer instructions, evidence block, reasoning context, output schema, grounding rules, citation rules, safety rules, response constraints, and audit metadata.

## Prompt Orchestrator

Subsystem that converts Reasoning Contexts into provider-independent Prompt Packages without invoking an LLM. See [Prompt Orchestration](architecture/prompt-orchestration.md).

## AI Task Orchestrator

Subsystem that routes prepared questions and contexts to AI task plugins and emits immutable AI Execution Plans. See [AI Task Orchestrator](architecture/task-orchestrator.md).

## AI Execution Plan

Immutable plan containing task routing, reasoning mode, prompt strategy, output schema, evaluation policies, safety constraints, and execution metadata.

## Prompt Strategy Engine

Registry-backed subsystem that selects provider-independent prompt strategy metadata for a task. See [Prompt Strategy Engine](architecture/prompt-strategy-engine.md).

## Output Schema Registry

Registry of structured response schemas selected by AI tasks. See [Output Schema Registry](architecture/output-schema-registry.md).

## Safety Policy Engine

Subsystem that attaches deterministic safety and evaluation rules before any future model execution. See [Safety Policy Engine](architecture/safety-policy-engine.md).

## LLM Runtime Engine

Provider-agnostic runtime that executes AI Execution Plans through model providers and returns raw responses. See [LLM Runtime](architecture/llm-runtime.md).

## Provider Adapter

Isolated implementation of one model provider behind the runtime provider contract.

## Model Selection Engine

Runtime component that selects a model from provider-independent metadata based on context, capabilities, cost, health, and overrides. See [Model Selection](architecture/model-selection.md).

## Token Accounting

Runtime accounting of estimated prompt tokens, actual prompt tokens, completion tokens, cached tokens, total tokens, and budget usage.

## Cost Accounting

Runtime accounting of estimated and actual provider/model cost.

## AI Quality Layer

Post-runtime subsystem that normalizes raw model output, validates grounding and citations, scores quality, creates validated reflections, and records feedback. See [Response Validation](architecture/response-validation.md).

## Validated AI Coach Response

Canonical response object accepted by the quality layer after schema, grounding, citation, recommendation, confidence, consistency, and quality checks.

## Grounding Validator

Quality-layer validator that verifies response observations and inferences are backed by deterministic evidence identifiers. See [Grounding Engine](architecture/grounding-engine.md).

## Citation Validator

Quality-layer validator that rejects citations not present in the Evidence Package or Reasoning Context.

## Recommendation Validator

Quality-layer validator that requires recommendations to be supported by evidence identifiers.

## Quality Report

Deterministic score report for a validated response, including grounding coverage, citation quality, recommendation support, actionability, readability, completeness, conciseness, contradictions, and overall quality.

## Reflection Memory

Validated, evidence-backed behavioral reflection store. It is not conversation memory. See [Reflection Memory](architecture/reflection-memory.md).

## Human Feedback Engine

Asynchronous subsystem that records user feedback about validated responses without mutating evidence. See [Human Feedback](architecture/human-feedback.md).

## Regeneration Policy

Declarative quality-layer decision indicating that a failed response should be regenerated by a future orchestration layer.
