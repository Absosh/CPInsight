...
function createSkillSimulation({ nodes, links, width, height, onTick }) {
  const simulation = simulation_default(nodes).force("link", link_default(links).id((datum) => datum.id).distance(180).strength(0.16)).force("charge", manyBody_default().strength(-150)).force("collide", collide_default(70)).force("center", center_default(width / 2, height / 2).strength(0.025)).force("anchor-x", x_default2((datum) => datum.anchorX).strength(0.05)).force("anchor-y", y_default2((datum) => datum.anchorY).strength(0.05)).alphaDecay(0.085).alphaMin(0.015).velocityDecay(0.58).on("tick", onTick).stop();
  simulation.updateCenter = (nextWidth, nextHeight) => {
    simulation.force("center", center_default(nextWidth / 2, nextHeight / 2).strength(0.025));
  };
  return simulation;
}
export {
  createSkillSimulation
};
